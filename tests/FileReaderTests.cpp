...
#include "../src/acquisition/FileReader.h"
...
TEST(FileReaderTestSuite, InitOK)
{
...
TEST(FileReaderTestSuite, NframesOK)
{
    auto srate_hz = std::stod(get_env_var("TEST_SRATE_HZ"));
    auto reader = make_file_reader<short>();

    EXPECT_EQ(unsigned (srate_hz * 60 * 10), reader.get_nframes());
}

// test that, given a binary file, acquire_frames returns the proper data
TEST(FileReaderTestSuite, AcquireFramesOK)
{
    auto reader = make_file_reader<short>();
    ...
    std::ifstream fp;
    fp.open(reader.get_filename());
    fp.read((char *) filebuf, sizeof(short) * n_channels * 5);
    fp.close();

    reader.acquire_frames(0, 5 * n_channels, framebuf);
